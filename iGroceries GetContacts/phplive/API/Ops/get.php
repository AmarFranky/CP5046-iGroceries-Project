<?php
					...
					$dept_trans )
	{
		...
		$query = "SELECT p_operators.opID AS opID, p_operators.lastactive AS lastactive, p_operators.maxc AS maxc, p_operators.mapp AS mapp, p_operators.ses AS ses, p_operators.rate AS rate, p_operators.sms AS sms, p_operators.smsnum AS smsnum, p_operators.name AS name, p_operators.email AS email FROM p_operators INNER JOIN p_dept_ops ON p_operators.opID = p_dept_ops.opID WHERE p_operators.status = 1 AND ( ( p_operators.lastactive > $lastactive ) OR ( p_operators.mapp <> 0 ) ) AND p_dept_ops.deptID = $deptid $rstring_query $order_by" ;
		database_mysql_query( $dbh, $query ) ;
