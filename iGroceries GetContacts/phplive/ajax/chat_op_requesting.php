<?php
	...
	if ( $action === "rq" )
	{
		...
		else
		{
			...
			$queue = Util_Format_Sanatize( Util_Format_GetVar( "qu" ), "n" ) ;
			...
			if ( $queue && !( $c_requesting % $VARS_CYCLE_CLEAN_Q ) )
			{
		...
		else
		{
			...
			for ( $c = 0; $c < count( $q_cces ); ++$c )
			{
				...
				else if ( !is_file( "$CONF[CHAT_IO_DIR]/$ces.txt" ) )
				{
					...
					$text = base64_encode( "<div class='cl'><disconnected><d5>".$LANG["CHAT_NOTIFY_DISCONNECT"]."</div>" ) ;
