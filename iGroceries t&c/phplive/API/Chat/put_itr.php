					...
					$deptvars )
	{
		...
		if ( $formatted && isset( $deptinfo["temail"] ) && isset( $requestinfo_log["custom"] ) )
		{
			if ( $status )
			{
				...
				$message_trans = Util_Email_FormatTranscript( $ces, $deptinfo["msg_email"], $deptinfo["name"], $deptinfo["email"], $vname, $vemail, $opinfo["name"], $opinfo["email"], $requestinfo_log["custom"], $formatted ) ;
				...
				if ( !$trans_exists )
				{
					...
					if ( $requestinfo_log["status"] && ( $requestinfo_log["status_msg"] == 1 ) && $vemail && ( $vemail != "null" ) )
					{
						$email_error = Util_Email_SendEmail( $from_name, $from_email, $vname, $vemail, $subject_visitor, $message_trans, "trans" ) ;
					}
