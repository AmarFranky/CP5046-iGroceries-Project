		...
		foreach ( $proactions_priority_array as $null => $this_proactionid )
		{
			...
			foreach ( $this_proaction_array as $this_key => $this_value )
			{
				...
				if ( ( $this_key == "profile" ) && $this_value )
				{
					...
					if ( $this_deptid )
					{
						foreach ( glob( "$CONF[CHAT_IO_DIR]/online_".$this_deptid."_*", GLOB_NOSORT ) as $file ) {
							preg_match( "/online_".$this_deptid."_(\d+)\.info/i", $file, $matches ) ;
					...
					else
					{
						foreach ( glob( "$CONF[CHAT_IO_DIR]/online_*", GLOB_NOSORT ) as $file ) {
							preg_match( "/online_(\d+)_(\d+)\.info/i", $file, $matches ) ;
