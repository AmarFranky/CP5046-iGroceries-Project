<?php
	...
	$profile_pic = "<div style='margin-bottom: 5px;'><table cellspacing=0 cellpadding=0 border=0><tr><td valign='top' width='55'><div id='chat_profile_pic'><img src='$profile_pic_url' width='55' height='55' border=0 class='profile_pic_img'></div></td><td valign='top' style='padding: 10px;'><div style='font-weight: bold; font-size: 14px;' id='chat_profile_name'>$opinfo_[name]</div><div style='margin-top: 4px;'>$deptinfo[name]</div><div style='margin-top: 4px;' class=''>Chat ID: $ces</div></div></td></tr></table></div>" ;
			...
			if ( function_exists( "Util_Emarketing_VisInfo" ) )
			{
				...
				if ( isset( $emarketinfo["md5_vis"] ) )
				{
					...
					if ( count( $emarketings ) )
					{
						...
						$emarketing_string = "<tr><td nowrap><img src='../addons/emarketing/pics/email_at.png' width='16' height='16' title='Email Marketing Opt-In' alt='Email Marketing Opt-In'> : </td><td>$emarketinginfo[message] <span style='font-weight: bold;'>$value_string</span></td></tr>" ;
					}
				}
			}
		}
	...
	$profile_pic .= "<div class='ca'><table cellspacing=0 cellpadding=2 border=0>$average_accept_string$visitorid_string$ip_string$tags_string$rating_string$comments_string$emarketing_string</table></div>" ;
	...
	if ( isset( $requestinfo["os"] ) && $requestinfo["md5_vis"] )
	{
		...
		<div id="chat_input" style="margin-top: 2px; padding: 5px; -moz-border-radius: 5px; border-radius: 5px;">
			...
			<tr>
				<td width="200" valign="top">
					<div class="chat_info_td_traffic" style="font-weight: bold;"><table cellspacing=0 cellpadding=0 border=0><tr><td style="padding-right: 5px;"><span id="chat_vtimer"></span></td><td><span id="req_rating"><?php echo $rating_stars ?></span> <img src="../pics/loading_pulse.gif" width="12" height="12" border="0" alt="real-time view" title="real-time view" style="display: none; background: #FFFFFF; padding: 2px; -moz-border-radius: 2px; border-radius: 2px;" id="img_spinner_real_time"></td></tr></table></div>
				...
				<td valign="top">
					...
					<tr>
						...
						<td><div class="chat_info_td_traffic"><?php echo ( $requestinfo["initiated"] ) ? "<img src=\"../themes/$CONF[THEME]/info_initiate.gif\" width=\"10\" height=\"10\" border=\"0\" alt=\"\" title=\"Operator Initiated Chat Invite\" alt=\"Operator Initiated Chat Invite\"> " : "" ; ?><b><?php echo ( $requestinfo["op2op"] ) ? "Operator to Operator Chat" : $opinfo_["name"] ; ?></b> <?php echo "<a href='mailto:$opinfo_[email]' target='new'>$opinfo_[email]</a>" ?></div></td>
					</tr>
					<tr>
...
<div class="info_neutral" style="position: fixed; width: 100%; bottom: 0px; padding: 10px; left: 0px; -moz-border-radius: 0px; border-radius: 0px; z-Index: 10;" id="div_options">
	...
	<?php if ( !$back ): ?>
	<div style="float: left; margin-left: 25px; cursor: pointer;" onClick="do_print('<?php echo $ces ?>', <?php echo $requestinfo["deptID"] ?>, <?php echo $requestinfo["opID"] ?>, <?php echo $VARS_CHAT_WIDTH+100 ?>, <?php echo $VARS_CHAT_HEIGHT+85 ?> )"><img src="../themes/<?php echo $theme ?>/printer.png" width="16" height="16" border="0" alt="print" title="print"></div>
	<?php endif ; ?>
...
<!--
	...
	$(document).ready(function()
	{
		...
		$('#chat_body').empty().html( "<?php echo $profile_pic ?>"+init_timestamps( transcript.emos().extract_youtube() ) ) ;
	...
	function realtime_update_ces( thejson_data )
	{
		...
		if ( ( typeof( chats[thisces] ) != "undefined" ) && ( parseInt( chats[thisces]["fline"] ) != parseInt( thejson_data["fline"] ) ) )
		{
			...
			$('#chat_body').append( init_timestamps( append_text.emos().extract_youtube() ) ).children(':last').hide().fadeIn(500) ;
			init_scrolling() ;
