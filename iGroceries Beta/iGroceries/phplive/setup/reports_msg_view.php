<?php
	...
	if ( isset( $message["messageID"] ) )
	{
		...
		$message_body = preg_replace( "/(\r\n)|(\n)|(\r)/", "<br>", Util_Format_ConvertQuotes( $message["message"] ) ) ;
	}
	else
	{
			...
			if ( custom_raw[c] != 0 )
			{
				...
				if ( custom_val[1] )
				{
					...
					if ( custom_value.match( /^http/ ) )
					{
						...
						custom_string += "<tr><td nowrap style=\"padding: 2px;\"><b>"+decodeURIComponent( custom_val[0] )+"</b></td><td style=\"padding: 2px; padding-left: 5px;\"><div style=\"padding-top: 0px;\" class=\"chat_info_td\" title=\""+custom_value+"\" alt=\""+custom_value+"\"><a href=\""+custom_value+"\" target=_blank>"+custom_value_snap+"</a></div></td></tr>" ;
					}
					else
						custom_string += "<tr><td nowrap style=\"padding: 2px;\"><b>"+decodeURIComponent( custom_val[0] )+"</b></td><td style=\"padding: 2px; padding-left: 5px;\">"+decodeURIComponent( custom_val[1] )+"</td></tr>" ;
				}
			}
		}
		custom_string += "</table>" ;
	...
	<div style="padding: 25px; background: #FAFAFA;">
		<div id="div_message_body" class="round">
			...
			<tr>
				...
				<td><div class="td_dept_td_blank" id="msg_created" style="padding: 4px; text-shadow: none;"><?php echo $created ?> from <b>IP:</b> <?php echo $message["ip"] ?></div></td>
			</tr>
			<tr>
				...
				<td valign="top">
					<div class="td_dept_td_blank" style="padding: 4px; text-shadow: none;">
						<div id="msg_message" style="padding: 25px; height: 150px; overflow: auto; border: 1px dashed #A9A9AA; word-break: break-word; word-wrap: break-word;" class="round"><?php echo $message_body ?></div>
					</div>
				</td>
			</tr>
			<tr>
