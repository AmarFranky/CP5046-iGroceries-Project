<?php
					...
					$sim_ops = "" )
	{
		...
		else
		{
			...
			$query = "INSERT INTO p_requests VALUES ( 0, $now, 0, 0, $now, $vupdated, $status, $auto_pop, $initiate, $deptid, $opid, $op2op, $t_vses, $marketid, 0, $os, $browser, $requests, 1, 0, '$ces', '$resolution', '$vname', '$vemail', '$ip', '$country', '$vis_token', '$vis_token_', '$sim_ops', '', '$onpage', '$title', '$rstring', '$refer', '$custom', '$question' )" ;
			database_mysql_query( $dbh, $query ) ;
					...
					$requestid )
	{
		...
		$query = "INSERT IGNORE INTO p_req_log ( ces, created, accepted, accepted_op, duration, ended, status, archive, status_msg, initiated, deptID, opID, op2op, marketID, os, browser, idle_disconnect, tag, resolution, vname, vemail, ip, md5_vis, sim_ops, onpage, title, custom, question ) SELECT ces, created, 0, 0, 0, 0, status, 0, 0, initiated, deptID, opID, op2op, marketID, os, browser, 0, 0, resolution, vname, vemail, ip, md5_vis_, sim_ops, onpage, title, custom, question FROM p_requests WHERE p_requests.requestID = $requestid" ;
		database_mysql_query( $dbh, $query ) ;
