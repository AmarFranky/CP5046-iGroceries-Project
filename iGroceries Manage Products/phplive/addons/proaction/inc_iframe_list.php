...
<!--
	...
	{
		if ( confirm( "Really reset the stats?" ) )
		{
			var unique = unixtime() ;
			...
			error:function (xhr, ajaxOptions, thrownError){
				parent.do_alert( 0, "Connection error.  Refresh the page and try again. [pr-e1]" ) ;
			} });
		}
	}
	...
	function update_status( theproid, thestatus )
	{
		if ( proids_status[theproid] != thestatus )
		{
			...
			error:function (xhr, ajaxOptions, thrownError){
				parent.do_alert( 0, "Connection error.  Refresh the page and try again. [pr-e2]" ) ;
			} });
		}
		else
			parent.do_alert( 1, "Update Success" ) ;
...
<body>
	<div id="div_proaction" style="">
		...
		<tr>
			...
			<td><div class="td_dept_header">Icon</div></td>
		...
		<?php
			...
			foreach ( $proactions_priority_array as $null => $thisproid )
			{
				...
				if ( isset( $depts_hash[$proaction_array["deptid"]] ) || isset( $dept_groups_hash[$proaction_array["deptid"]] ) )
				{
					...
					$edit_delete = "<div style=\"cursor: pointer;\" onClick=\"parent.do_edit( '$thisproid', $proaction_array[deptid], $proaction_array[position], $proaction_array[duration], $proaction_array[andor], $proaction_array[footprints], $proaction_array[reset], $proaction_array[profile], '$proaction_array[exin]', '$exclude', $proaction_array[onoff], '$proaction_array[click]', '".rawurlencode( $proaction_array["message"] )."' )\"><img src=\"../../pics/btn_edit.png\" width=\"64\" height=\"23\" border=\"0\" alt=\"\"></div><div onClick=\"parent.do_delete( '$thisproid' )\" style=\"margin-top: 10px; cursor: pointer;\"><img src=\"../../pics/btn_delete.png\" width=\"64\" height=\"23\" border=\"0\" alt=\"\"></div>" ;
					...
					print '<tr id="tr_'.$array_index.'" bgColor="'.$tr_color.'">
						...
						<td style="" class="td_dept_td" valign="top">
							<span style="font-weight: bold; text-shadow: none;" class="info_blue_dark" alt="display this ProAction for HTML Code: '.$department.'" title="display this ProAction for HTML Code: '.$department.'"><img src="../../pics/icons/dept.png" width="14" height="14" border="0" alt=""> '.$department.'</span>
							<table cellspacing=0 cellpadding=0 border=0><tr>
								<td><div id="div_message_'.$thisproid.'" style="position: relative; background: #FFFFFF; padding: 10px; -moz-border-radius: 5px; border-radius: 5px; box-shadow: 3px 3px 15px rgba(0, 0, 0, 0.2); width: 150px; height: 75px; overflow: hidden;"><div>'.$message.'</div><div style="position: absolute; top: 10px; left: 0px; width: 170px; height: 85px; background: transparent; cursor: pointer;" onClick="parent.view_demo(\''.$thisproid.'\', '.$proaction_array["position"].', '.$proaction_array["profile"].', \''.rawurlencode( $proaction_array["message"] ).'\')" title="view invite preview" alt="view invite preview"></div></div><div style="margin-top: 5px; font-size: 10px; color: #A8A8A8;">ID: '.$thisproid.'</div></td>
						...
						<td class="td_dept_td">on same page '.$duration.' <div style="margin-top: 10px; text-shadow: none;">'.$andor.' '.$footprints.' footprints</div>
							<div style="margin-top: 15px;" class="info_neutral">click behavior: '.$click_behavior.'</div>'.$exclude_string.'
						</td>
