<?php
	...
	$op_depts = Depts_get_OpDepts( $dbh, $opid ) ; $op_depts_queue_enabled = 0 ;
	...
	for ( $c = 0; $c < count( $op_depts ); ++$c )
	{
		...
		if ( $department["queue"] ) { $op_depts_queue_enabled = 1 ; }
	...
	if ( is_file( "$CONF[TYPE_IO_DIR]/$opid.mapp" ) )
	{
		...
		if ( isset( $VARS_MAPP_DEBUG ) ) { Util_Format_DEBUG( "[ mapp ] [x] $opid - operator.php [1]\r\n" ) ; }
	}
	...
	if ( is_file( "$CONF[CONF_ROOT]/autotask_config.php" ) && is_file( "$CONF[DOCUMENT_ROOT]/addons/autotask/op_init.php" ) && isset( $VALS["AUTOTASK"] ) && $VALS["AUTOTASK"] )
	{ include_once( "$CONF[DOCUMENT_ROOT]/addons/autotask/op_init.php" ) ; }
	...
	$proactions_string = "" ;
	if ( isset( $VALS_ADDONS["proaction"] ) )
	{
...
<!--
	...
	if ( phplive_session_support )
	{
		try {
			// initial check
			var temp = localStorage.getItem("volume_newrequest") ;
			if ( temp ) { sound_volume = temp ; }
		} catch (error) {}
	}
	...
	var si_offline, si_title, si_typing, si_rating, si_automatic_offline, si_textarea, si_istyping_clear ;
	...
	var autotask_queueids_select = "" ; var autotask_statuses_select = "" ; var autotask_priorities_select = "" ; var autotask_worktypeids_select = "" ; var autotask_notetypes_select = "" ;
	var autotask_queueids_select_orig = "" ; var autotask_statuses_select_orig = "" ; var autotask_priorities_select_orig = "" ; var autotask_worktypeids_select_orig = "" ; var autotask_notetypes_select_orig = "" ;
	...
	var dc_c_queueing = 0 ; var depts_queue_enabled = <?php echo $op_depts_queue_enabled ?> ;
	...
	$(document).ready(function()
	{
		...
		*/
		...
		<?php if ( $reload && !$nalert && !$salert ): ?>do_alert( 1, "Success" ) ;<?php endif ; ?>
	...
	function prep_init()
	{
		...
		if ( profile_pic_enabled ) { $('#div_profile_pic').empty().html('<img src="<?php echo $profile_pic_url ?>" width="55" height="55" border=0 class="profile_pic_img" id="img_profile_pic"><div style="margin-top: 5px; font-size: 10px;">profile</div>').fadeIn( "fast" ) ; }
	...
	function new_chat( thejson_data, theflag )
	{
		...
		if ( typeof( chats[thisces] ) == "undefined" )
		{
			if ( !is_in_his && ( typeof( cl_his[thisces] ) == "undefined" ) )
			{
				...
				chats[thisces]["tupdated"] = thejson_data["tupdated"] ;
				...
				{
					var transferred_string = ( chats[thisces]["tupdated"] == 2 ) ? "<div style=\"margin-top: 10px;\"><div class=\"ctitle\">Transferred Chat</div></div>" : "" ;
					chats[thisces]["trans"] = "<c615><div class=\"ca\"><div class=\"info_box\"><i>"+thejson_data["question"]+"</i></div>"+transferred_string+"<div style=\"margin-top: 10px;\"><div class=\"ctitle\">"+depts_hash[chats[thisces]["deptid"]]+"</div> <div style=\"margin-top: 10px;\"><button type=\"button\" style=\"padding: 10px;\" class=\"input_op_button\" onClick=\"$(this).attr('disabled', 'true');chat_accept();\">accept</button> &nbsp; or &nbsp; <span onClick=\"chat_decline()\" style=\"text-decoration: underline; cursor: pointer;\">decline</span></div></div></c615>" ;
		...
		else if ( ( chats[thisces]["status"] == 3 ) && ( !thejson_data["status"] || ( thejson_data["status"] == 2 ) ) )
		{
			...
			if ( thisces == ces )
			{
				...
				$('#chat_body').empty().html( transcript.emos() ).children(':last').hide().fadeIn(500) ;
				init_scrolling() ;
			}
			else
	...
	function init_chat_list( theflag )
	{
		...
		for ( var c = 0; c < obj_length; ++c )
		{
			...
			var icon_op2op = ( ( chats[thisces]["status"] != 2 ) && chats[thisces]["op2op"] ) ? " <img src=\"../themes/"+theme+"/agent.png\" width=\"16\" height=\"16\" border=\"0\" title=\"Operator-to-Operator chat\" alt=\"Operator-to-Operator chat\"> " : "" ;
	...
	function activate_chat( theces )
	{
		...
		if ( typeof( chats[ces] ) != "undefined" )
		{
			...
			$('#chat_body').empty().html( init_timestamps( transcript.emos().extract_youtube() ) ) ;
			...
			if ( !chats[ces]["op2op"] || ( chats[ces]["op2op"] && ( chats[ces]["status"] == 2 ) ) )
			{
				...
				var req_auto_pop = ( chats[ces]["auto_pop"] ) ? "<img src=\"../themes/"+theme+"/info_flag.gif\" width=\"10\" height=\"10\" border=\"0\" alt=\"pre-populated visitor information\" title=\"pre-populated visitor information\"> " : "" ;
				...
				if ( url_raw == "livechatimagelink" ) { url_raw = "#" ; }
				...
				var geomap_string_display = ( mapp ) ? chats[ces]["ip"]+geomap_string : "<a href=\"#\" onClick=\"toggle_info_maps()\">"+chats[ces]["ip"]+geomap_string+"</a>" ;
				...
				$('#req_onpage').empty().html( "<div title=\""+chats[ces]["onpage"]+"\"><a href=\""+url_raw+"\" target=\"_blank\" alt=\""+chats[ces]["onpage"]+"\" title=\""+chats[ces]["onpage"]+"\">"+chats[ces]["title"]+"</a></div>" ) ;
				$('#req_refer').empty().html( "<a href=\""+chats[ces]["refer_raw"]+"\" target=\"_blank\" alt=\""+chats[ces]["refer_raw"]+"\" title=\""+chats[ces]["refer_raw"]+"\">"+chats[ces]["refer_snap"]+"</a>"+"&nbsp;" ) ;
				$('#req_market').empty().html( marketing+"&nbsp;" ) ;
				$('#req_resolution').empty().html( chats[ces]["resolution"]+" &nbsp; <img src=\"../themes/"+theme+"/os/"+chats[ces]["os"]+".png\" border=0 alt=\""+chats[ces]["os"]+"\" title=\""+chats[ces]["os"]+"\" alt=\""+chats[ces]["os"]+"\" width=\"10\" height=\"10\"> &nbsp; <img src=\"../themes/"+theme+"/browsers/"+chats[ces]["browser"]+".png\" border=0 alt=\""+chats[ces]["browser"]+"\" title=\""+chats[ces]["browser"]+"\" alt=\""+chats[ces]["browser"]+"\" width=\"10\" height=\"10\">" ) ;
				$('#req_ip').empty().html( geomap_string_display ) ;
				$('#req_t_ses').empty().html( "("+chats[ces]["t_ses"]+")" ).fadeIn( "fast" ) ;
				$('#req_vid').empty().html( chats[ces]["vis_token"] ) ;
			...
			// need to init menu_info hover when tabs appear
			init_info() ;
		}
		disconnect_showhide() ;
	}

	var menu_info_hover_init = false ;
	function init_info()
	{
		if ( !menu_info_hover_init )
		{
			...
			if ( custom_raw[c] != 0 )
			{
				...
				if ( custom_val[1] )
				{
					...
					if ( custom_value.match( /^((http)|(www))/ ) )
					{
						...
						custom_string += "<tr><td><div class=\"chat_info_td_blank\"><span style=\"font-weight: bold;\">"+custom_val[0]+": </span> <span title=\""+custom_value+"\" alt=\""+custom_value+"\"><a href=\""+custom_value+"\" target=\"_blank\">"+custom_value_snap+"</a></span></div></td></tr>" ;
					}
					...
					else
					{
						if ( autotask && autotask_dlink_ticketid && ( ( custom_val[0] == autotask_custom_var ) || ( chats[ces]["addon_at_ticketid"] ) ) )
						{
							...
							custom_string += "<tr><td><div class=\"chat_info_td_blank\"><span style=\"font-weight: bold;\">"+custom_val[0]+": </span> <span><a href=\""+autotask_dlink_ticketid+chats[ces]["addon_at_ticketid"]+"\" target=\"_blank\">"+chats[ces]["addon_at_ticketid"]+"</a></span></div></td></tr>" ;
						}
						else
							custom_string += "<tr><td><div class=\"chat_info_td_blank\"><span style=\"font-weight: bold;\">"+custom_val[0]+": </span> <span>"+custom_val[1]+"</span></div></td></tr>" ;
					}
				...
				{
					custom_string += "<tr><td><div class=\"chat_info_td_blank\"><span style=\"font-weight: bold;\">"+custom_val[0]+": </span> <span><a href=\"#\" onClick=\"toggle_info('addon_at',1)\"><img src=\"../pics/icons/autotask.png\" width=\"20\" height=\"16\" border=\"0\" alt=\"\" class=\"round\" style=\"background: #FFFFFF;\"></a></span></div></td></tr>" ;
				}
				custom_display = 1 ;
			}
		}
			...
			success: function(data){
				...
				if ( json_data.status )
				{
					...
					if ( json_data.tooslow )
					{
						...
						chats[ces]["tooslow"] = 1 ;
						chats[ces]["trans"] = "<div class='cl'><div class=\"ctitle\">Chat request has expired.</div><b>Possible Reasons:</b> The chat request does not exist, was accepted by another operator or the visitor closed the request.<div style='margin-top: 5px;'><button onClick='cleanup_disconnect(ces)' style='padding: 10px;' class='input_op_button'>close</button></div></div>" ;
						$('#chat_body').empty().html( chats[ces]["trans"] ).children(':last').hide().fadeIn(500) ;
					}
					else
					{
						if ( chats[ces]["status"] == 2 )
						...
						if ( auto_canid && ( chats[ces]["status"] != 2 ) && !chats[ces]["op2op"] )
						{
							...
							if ( can_exists )
							{
								...
								setTimeout( function(){ add_text_prepare(1) ; }, 3300 ) ;
							}
						}
					}
				}
		...
		success: function(data){
			...
			if ( json_data.status )
			{
				...
				<?php
					...
					print "if ( ( ".count( $op_depts )." > 1 ) || parseInt( json_data.global ) ) { \$('#canned_select').append('<optgroup label=\"All Departments\">') ; print_dept_cats_select( 1111111111 ) ; $('#canned_select').append('</optgroup>') ; global_can_exists = 1 ; } " ;
				?>
			}
			else
		...
		if ( typeof( ces ) == "undefined" )
		{
			if ( !prev_status )
			{
				if ( !chat_body_content.match( /notice_online_tag/i ) )
					$('#chat_body').empty().html("<div class='notice_online_tag' style=\"padding: 10px;\">You are <span class=\"info_good\">ONLINE</span>.<div style='margin-top: 20px;'>To receive visitor chat requests, keep this window open or minimized.</div></div>").children(':last').hide().fadeIn(500) ;
			}
			else
			{
				if ( automatic_offline_active && !chat_body_content.match( /notice_automatic_offline/i ) )
				{
					$('#chat_body').empty().html("<div class='info_error notice_automatic_offline'>You are OFFLINE.</div><div class='info_error' style='margin-top: 15px;' id='div_automatic_offline'><img src='../themes/"+theme+"/alert.png' width='16' height='16' border='0' alt=''> <span style='font-size: 16px; font-weight: bold;'>Offline Hours</span><div style='margin-top: 10px;'>It is past regular chat support hours.  The system is set to automatically logout shortly and will be available again during regular chat support hours.  <div style='margin-top: 15px;'><button type='button' class='input_button' style='padding: 10px;' onClick='ao=0;rd=0;dup=0;mi=0;toggle_status(3)'>Continue and logout.</button></div></div></div>").children(':last').hide().fadeIn(500) ;
				}
				else if ( !chat_body_content.match( /offline/i ) )
					$('#chat_body').empty().html("<div class='info_error'>You are OFFLINE.</div>").children(':last').hide().fadeIn(500) ;
			}
		}
		...
		for ( var c = 0; c < obj_length; ++c )
		{
			if ( divs[c] != thediv )
			{
				...
				if ( ( divs[c] != "info" ) && ( divs[c] != "maps" ) && ( divs[c] != "addon_at" ) )
					$('#info_'+divs[c]).empty().html( "<img src=\"../themes/"+theme+"/loading_fb.gif\" border=\"0\" alt=\"\">" ) ;
	...
	function disconnect_showhide()
	{
		if ( ( typeof( ces ) != "undefined" ) && ( typeof( chats[ces] ) != "undefined" ) )
		{
			if ( !chats[ces]["closed"] )
			{
				$('#info_disconnect').css({"padding": "3px"}).empty().html( "<img src=\"../themes/"+theme+"/close_extra.png\" width=\"14\" height=\"14\" border=\"0\" alt=\"\"> close chat with <b>"+chats[ces]["vname"]+"</b>" ) ;
				if ( mapp ) { $('#info_disconnect_mapp').empty().html( "<img src=\"../themes/"+theme+"/close_extra.png\" width=\"14\" height=\"14\" border=\"0\" alt=\"\"> close chat with <b>"+chats[ces]["vname"]+"</b>" ).fadeIn( "fast" ) ; $('#options_mapp_vinfo').fadeIn( "fast" ) ; }
			}
		}
		else
		{
		...
		success: function(data){
			...
			if ( json_data.status )
			{
				...
				for ( var c = 0; c < json_length; ++c )
				{
					...
					var btn_transfer_department = ( total_dept_ops ) ? "&nbsp;<button type=\"button\" onClick=\"transfer_chat( "+json_data.departments[c]["deptid"]+",'"+json_data.departments[c]["name"]+"', 0, 0, 0);$(this).attr('disabled', 'true');\" style=\"font-size: 10px;\">transfer</button>" : "" ;
					ops_string += "<div class=\"chat_info_td_h\"><b>"+json_data.departments[c]["name"]+" %%btn_transfer_department%%</b></div>" ;
					...
					for ( var c2 = 0; c2 < total_dept_ops; ++c2 )
					{
						...
						if ( json_data.departments[c].operators[c2]["status"] )
						{
							...
							btn_transfer = "<button type=\"button\" onClick=\"transfer_chat( "+json_data.departments[c]["deptid"]+",'"+json_data.departments[c]["name"]+"',"+json_data.departments[c].operators[c2]["opid"]+",'"+json_data.departments[c].operators[c2]["name"]+"', 0);$(this).attr('disabled', 'true');\" style=\"font-size: 10px;\">transfer</button>" ;
						}
						...
						else
						{
							...
							ops_string += "<div class=\"chat_info_td\" style=\"padding-left: 15px;\"><img src=\"../themes/"+theme+"/"+status_bullet+"\" width=\"12\" height=\"12\" border=\"0\"> "+btn_transfer+" "+json_data.departments[c].operators[c2]["name"]+" is "+status+chatting_with+"</div>" ;
						}
					}
	...
	function transfer_chat( thedeptid, thedeptname, theopid, theopname, themapp )
	{
		var unique = unixtime() ;
		var json_data = new Object ;
		...
		if ( chats[ces]["status"] != 3 )
		{
			...
			success: function(data){
				...
				if ( json_data.status )
				{
					...
					var trans_string = "<c615><div class='cl'>Chat has been transferred to <b>"+trans_to+"</b>. Chat has ended.<div style='margin-top: 5px;'><button onClick='cleanup_disconnect(ces)' style='padding: 10px;' class='input_op_button'>close chat</button></div></div></c615>" ;
	...
	function populate_settings( themenu )
	{
		...
		// settings load themes area for now
		if ( themenu == "settings" )
			theurl = "./settings.php?menu=themes&auto=<?php echo $auto ?>&wp=1&console=1&"+unixtime()+"&" ;
			...
			theurl = "./transcripts.php?auto=<?php echo $auto ?>&wp=1&console=1&"+unixtime()+"&" ;
	...
	function toggle_extra( thediv, theflag, theurl, thename, themenu )
	{
		...
		else
		{
			...
			var span_loading = ( mapp || ( typeof( thediv ) == "number" ) ) ? "<img src=\"../pics/space.gif\" width=\"16\" height=\"11\" border=\"0\" alt=\"\">" : "<img src=\"../themes/"+theme+"/loading_fb.gif\" border=\"0\" alt=\"\">" ;
			$('#chat_extra_title').html( "<span onClick=\"close_extra( '"+extra+"' )\" class=\"info_error\" style=\"font-weight: normal; cursor: pointer;\">close</span> <span id=\"span_extra_close\">"+span_loading+"</span> <span style=\"padding: 5px;\"><img src=\"../pics/space.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"\"></span>" + thename ) ;
			if ( thediv == "op2op" )
			{
			...
			else if ( thediv == "traffic" )
			{
				...
				$('#chat_extra_title').append( traffic_sound_onoff+" &nbsp; <span style=\"font-size: 10px; font-weight: normal;\">Traffic monitor will refresh every 60 seconds or when there is change in traffic. <button type='botton' style='font-size: 10px; padding: 4px;' onClick='init_reload_traffic(\"iframe_traffic\")' class=\"input_op_button\">refresh now</button></span>" ) ;
	...
	{
		var divs = Array( "op2op", "traffic", "canned", "trans", "at", "extras" ) ;
		var obj_length = divs.length ;
		for ( var c = 0; c < obj_length; ++c )
	...
	function close_extra_doit( thediv )
	{
		if ( typeof( thediv ) != "number" )
		{
			if ( ( thediv == "op2op" ) || ( thediv == "canned" ) || ( thediv == "trans" ) || ( thediv == "traffic" ) || ( thediv == "settings" ) || ( thediv == "mapp_themes" ) || ( thediv == "mapp_prefs" ) || ( thediv == "mapp_sounds" ) || ( thediv == "mapp_cans" ) || ( thediv == "mapp_trans" ) || ( thediv == "mapp_operators" ) || ( thediv == "mapp_power" ) || ( thediv == "mapp_traffic" ) ) { iframe_blank( thediv ) ; }
		}
		extra = undeefined ;
		reset_footer() ;
	}
	...
	function hide_extra( thediv )
	{
		...
		for ( var c = 0; c < obj_length; ++c )
		{
			if ( divs[c] != thediv )
			{
				...
				if ( ( thisdiv == "op2op" ) || ( thisdiv == "canned" ) || ( thisdiv == "trans" ) || ( thisdiv == "traffic" ) || ( thisdiv == "settings" ) || ( thisdiv == "mapp_themes" ) || ( thisdiv == "mapp_prefs" ) || ( thisdiv == "mapp_sounds" ) || ( thisdiv == "mapp_cans" ) || ( thisdiv == "mapp_trans" ) || ( thisdiv == "mapp_operators" ) || ( thisdiv == "mapp_power" ) || ( thisdiv == "mapp_traffic" ) ) { iframe_blank( thisdiv ) ; }
	...
	function select_canned_pre( theauto_canid )
	{
		...
		if ( typeof( thetitle ) != "undefined" )
		{
			$('#canned_select option').filter(function () {
				...
				if ( temp_title == thetitle.replace( /&-#39;/g, "'" ) )
				{
					this.selected = true ;
				}
			}) ;
	...
	function toggle_offline_timer_showhide( theflag )
	{
		if ( theflag )
		{
			...
			$('#offline_timer_description').html( "You have reached the max concurrent chat limit.  System will automatically place you back online when your concurrent chat total is below "+maxc+" chat session(s)." ) ;
			$('#offline_timer').hide() ;
	...
	function update_ratings_doit()
	{
		...
		var rating_flag = ( fetch_rating_flag >= 10 ) ? 0 : 1 ;
			...
			if ( json_data.status )
			{
				...
				else
				{
					...
					if ( rating_flag )
					{
						...
						$('#rating_recent').html( stars_string ).unbind('click').bind('click', function() {
							if ( json_data.ces )
	...
	function launch_alerts()
	{
		...
		$('#div_alerts_wrapper').css({'width': div_width}) ;
			...
			if ( json_data.status )
			{
				...
				for ( var c = 0; c < json_length; ++c )
				{
					...
					if ( !is_in_his )
					{
						...
						var vemail = ( declineinfo["vemail"] != "null" ) ? " <a href=\"mailto:"+declineinfo["vemail"]+"\">"+declineinfo["vemail"]+"</a>" : "" ;
						...
						declines_string += "<tr><td><div class=\"chat_info_td_t\"><span style=\"cursor: help;\">"+status_icon+status_msg_icon+"</span>"+created+"</div></td></tr><tr><td class=\"chat_info_td_traffic\"><table cellspacing=0 cellpadding=2 border=0 width=\"100%\">"+
							...
							"<tr><td class=\"chat_info_td_h\">Clicked From</td><td class=\"chat_info_td\">"+onpage+"</td></tr>"+
							"<tr><td class=\"chat_info_td_h\" valign=\"top\">Question</td><td class=\"chat_info_td\">"+question+custom_vars_string+"</td></tr></table></td></tr>" ;
					}
				}
	...
	{
		var url = "./op_trans_view.php?ces="+theces+"&id="+isop+"&wp="+wp+"&auth=op&"+unixtime()+"&" ;
		...
		else
		{
			...
			var this_regex = /(.*?)operator.php/i ;
			var matches = this_regex.exec( full_url ) ;

			if ( ( typeof( matches[1] ) != "undefined" ) && ( full_url.match( /^http/i ) ) )
			{
				url = matches[1]+"op_trans_view.php?ces="+theces+"&id="+isop+"&wp="+wp+"&auth=op&"+unixtime()+"&" ;
				wp_new_win( url, "Transcript_"+theces, <?php echo $VARS_CHAT_WIDTH+120 ?>, <?php echo $VARS_CHAT_HEIGHT+85 ?> ) ;
			}
			else
				do_alert( 0, "Could not parse the URL." ) ;
		}
	...
	{
		if ( traffic_sound )
			$('#div_traffic_sound').empty().html( "<img src=\"../themes/"+theme+"/bell_start.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"\"> traffic sound is ON" ).removeClass('sound_box_off').addClass('sound_box_on') ;
		else
			$('#div_traffic_sound').empty().html( "<img src=\"../themes/"+theme+"/bell_stop.png\" width=\"16\" height=\"16\" border=\"0\" alt=\"\">  traffic sound is OFF" ).removeClass('sound_box_on').addClass('sound_box_off') ;
	}
	...
	function reconnect_doit()
	{
		...
		var reconnect_cycles = ( mapp ) ? 2000 : 60 ;
		if ( reconnect_counter > reconnect_cycles )
		{
			...
			$('#reconnect_status').empty().html("<img src=\"../themes/"+theme+"/alert.png\" width=\"14\" height=\"14\" border=\"0\" alt=\"\"> Could not reconnect.  "+a_message) ;
	...
	function update_theme( thetheme, thetheme_timestamp )
	{
		...
		$('html, body').animate({
			...
			// rever to info tab due to iframe maps reset
			toggle_info('info',1) ;
...
<body style="display: none; overflow: hidden; -webkit-text-size-adjust: 100%;" onClick="clear_flash_console();init_nsleep();">
...
<div id="chat_body_wrapper" style="position: absolute; top: 20px; padding: 10px; z-Index: 2;" onClick="input_focus();dn_close();<?php if ( !$mapp ): ?>close_extra( extra );<?php endif ; ?>">
	<div id="chat_body" style="overflow: auto; -webkit-overflow-scrolling: touch; <?php echo ( $mobile || $mapp ) ? " padding: 5px;": "" ; ?> word-break: break-word; word-wrap: break-word;" onClick="close_misc('all')"><div class="info_neutral" style="display: inline-block; *display: inline; zoom: 1; margin-top: 5px; padding: 25px;"><img src="../pics/spinner.gif" width="16" height="16" border="0" alt="" class="round" title="connecting..." alt="connecting..."> loading</div></div>
	<div id="chat_options" style="padding: 5px;">
		<div style="height: 16px;">
			<div id="options_themes" style="float: left; padding-right: 15px; cursor: pointer;" onClick="launch_settings('themes')" id="chat_settings" alt="" title=""><table cellspacing=0 cellpadding=0 border=0><tr><td><img src="../themes/<?php echo $theme ?>/vcard.png" width="16" height="16" border="0"></td><td>&nbsp;themes</td></tr></table></div>
			<?php if ( !$mapp ): ?>
			<div id="options_trans" style="float: left; padding-right: 15px; cursor: pointer;" onClick="launch_settings('trans')" alt="" title=""><table cellspacing=0 cellpadding=0 border=0><tr><td><img src="../themes/<?php echo $theme ?>/view.png" width="16" height="16" border="0"></td><td>&nbsp;transcripts</td></tr></table></div>
			<?php endif ; ?>
			<div id="options_sound" style="float: left; padding-right: 15px; cursor: pointer;" onClick="launch_settings('sounds')" title="" alt=""><table cellspacing=0 cellpadding=0 border=0><tr><td><img src="../themes/<?php echo $theme ?>/sound_on.png" width="16" height="16" border="0" alt="" id="chat_sound"></td><?php if ( !$mapp ): ?><td>&nbsp;sound</td><?php endif ; ?></table></div>
	...
	<div id="chat_input" style="margin-top: 8px;" onClick="dn_close();">
		<div style="display: none; float: left; width: 65px; cursor: pointer; text-align: center;" id="div_profile_pic" onClick="launch_settings('profile')"></div>
				...
				<div id="chat_info_body" style="overflow: auto;">
					<div id="info_info" style="display: none; text-align: justify;">
						<div id="info_info_body" style="">
							...
							<tr><td class="chat_info_td_h" style="opacity: 0.5; filter: alpha(opacity=50);">Department</td><td width="100%" class="chat_info_td" style="opacity: 0.5; filter: alpha(opacity=50);"> <span id="req_dept" style="font-weight: bold;"></span></td></tr>
							...
							<tr><td nowrap class="chat_info_td_h" nowrap style="padding-top: 10px; padding-bottom: 10px;">Custom Vars</td><td class="chat_info_td"><div id="req_custom" style="max-height: 80px; overflow-y: auto; overflow-x: hidden;"></div></td></tr>
							...
							<tr><td class="chat_info_td_h" nowrap style="opacity: 0.5; filter: alpha(opacity=50);">Visitor ID</td><td class="chat_info_td" style="opacity: 0.5; filter: alpha(opacity=50);"> <span id="req_vid" style=""></span></td></tr>
							<tr><td class="chat_info_td_h" style="opacity: 0.5; filter: alpha(opacity=50);">Chat ID</td><td class="chat_info_td" style="opacity: 0.5; filter: alpha(opacity=50);"> <span id="req_ces" style="font-weight: bold;"></span> &nbsp; <span id="req_t_ses" style="display: none;"></span></td></tr>
							</table>
						</div>
					</div>
...
<div id="chat_footer" style="position: absolute; width: 100%; bottom: 0px; height: 34px; overflow: hidden; z-Index: 106;">
	...
	<?php if ( !$mapp && $addon_autotask ): ?>
	<div class="chat_footer_cell_noclick"><img src="../themes/<?php echo $theme ?>/divider.png" border="0" alt=""></div>
	<div id="chat_footer_cell_at" class="chat_footer_cell" onClick="toggle_extra( 'at', '', '', 'Autotask' )"> <img src="../pics/icons/autotask.png" width="20" height="16" border="0" alt="Autotask" title="Autotask" class="round" style="background: #FFFFFF;"></div>
	<?php endif ; ?>
...
<div id="chat_extra_wrapper" style="position: absolute; display: none; bottom: 0px; width: 100%; overflow: auto; z-Index: 105;">
...
<div id="info_disconnect" class="info_disconnect" style="position: absolute; top: 1px; right: 0px; text-align: right; z-Index: 601;" onClick="pre_disconnect();"></div>
...
<div id="reconnect_notice" class="info_warning" style="display: none; position: absolute; z-Index: 1000; -webkit-box-shadow: -0px 7px 29px rgba(0, 0, 0, 0.34); -moz-box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34); box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34);">
	<div id="reconnect_status">Operator console disconnected.  Reconnecting... <img src="../themes/<?php echo $theme ?>/loading_fb.gif" border="0" alt=""></div>
...
<div style="display: none; position: absolute; top: 0px; left: 0px; padding: 45px; z-Index: 102; cursor: pointer; -webkit-box-shadow: -0px 7px 29px rgba(0, 0, 0, 0.34); -moz-box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34); box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34);" class="info_content" id="div_last_response" onClick="toggle_last_response(0)">
...
<div id="div_debug" style="display: none; position: absolute; bottom: 0px; right: 0px; width: 400px; height: 200px; overflow: auto; z-Index: 1000; -webkit-box-shadow: -0px 7px 29px rgba(0, 0, 0, 0.34); -moz-box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34); box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34);" class="info_content">
	<form>
...
<div id="div_mapp_spinner" style="display: none; position: absolute; top: 0px; left: 0px; z-Index: 11;"><span class="info_neutral" style="padding: 25px; -webkit-box-shadow: -0px 7px 29px rgba(0, 0, 0, 0.34); -moz-box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34); box-shadow: 0px 7px 29px rgba(0, 0, 0, 0.34);"><img src="../pics/spinner.gif" width="16" height="16" border="0" alt="" class="round" title="connecting..." alt="connecting..." id="img_mapp_spinner"> connecting</span></div>
<div id="div_alerts_wrapper" style="display: none; position: absolute; bottom: 0px; right: 0px; width: 550px; height: 515px; z-Index: 601; border-bottom-left-radius: 0px 0px; -moz-border-radius-bottomleft: 0px 0px; border-bottom-right-radius: 0px 0px; -moz-border-radius-bottomright: 0px 0px; border-top-right-radius: 0px 0px; -moz-border-radius-topright: 0px 0px;" class="info_content">
	<div style="padding: 15px;">
		<div class="info_error" onClick="launch_alerts()" style="text-align: center; cursor: pointer;"><img src="../themes/<?php echo $theme ?>/close_extra.png" width="16" height="16" border="0"> Close</div>
		<div style="margin-top: 15px;"><img src="../themes/initiate/notice.png" width="24" height="24" border="0" alt="Alert" title="Alert"> Your declined (and missed) chat requests in recent 24 hours:</div>
	</div>
	<div id="div_alerts" style="padding: 15px; padding-top: 0px; height: 330px; overflow: auto; word-break: break-word; word-wrap: break-word;"><img src="../themes/<?php echo $theme ?>/loading_fb.gif" border="0" alt=""></div>
...
<div id="div_sound_interaction_message" class="info_content" style="display: none; position: absolute; top: 0px; left: 0px; width: 310px; padding: 25px; text-align: center; z-Index: 1001;">
	...
	<div style="margin-top: 15px;">
		Some web browsers require user interaction on the page to play sound. Click the 'Ok' button to ensure sound is enabled for this session.
	</div>
	...
	<div style="margin-top: 15px;"><button type="button" class="input_button" style="padding: 10px;" onClick="close_sound_interaction()" id="div_sound_interaction_message_btn">Ok</button></div>
</div>
