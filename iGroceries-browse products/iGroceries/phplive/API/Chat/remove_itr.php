<?php
	...
	FUNCTION Chat_remove_itr_OldRequests( &$dbh )
	{
		...
		$query = "SELECT SQL_NO_CACHE * FROM p_requests WHERE ( ( updated < $expired OR ( vupdated < $expired AND vupdated <> 615 ) ) OR ( status = 1 AND ( updated < $expired_active OR vupdated < $expired_active ) ) ) AND op2op = 0" ;
		...
		if ( $dbh[ 'ok' ] )
		{
			if ( database_mysql_nresults( $dbh ) || $total_dir_files )
			{
				...
				if ( count( $expired_requests ) )
				{
					...
					for ( $c = 0; $c < count( $expired_requests ); ++$c )
					{
						...
						if ( is_file( "$CONF[CHAT_IO_DIR]/$trans_file" ) )
						{
							...
							if ( !isset( $transcript["ces"] ) )
							{
								...
								$filename_op = $ces."-$request[opID]" ; UtilChat_AppendToChatfile( "$filename_op.text", base64_encode( $string_disconnect ) ) ;
								...
								for ( $c2 = 1; $c2 <= $max_vses; ++$c2 )
								{
									...
									UtilChat_AppendToChatfile( "$filename_vis.text", base64_encode( $string_disconnect ) ) ;
								}
					...
					$query = "DELETE FROM p_requests WHERE ( ( updated < $expired OR ( vupdated < $expired AND vupdated <> 615 ) ) OR ( status = 1 AND ( updated < $expired_active OR vupdated < $expired_active ) ) ) AND op2op = 0" ;
					database_mysql_query( $dbh, $query ) ;
				}
			}
