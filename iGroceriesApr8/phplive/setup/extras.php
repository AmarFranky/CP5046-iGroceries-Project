...
<!--
	...
	function do_options( theoption, theextid, thename, theurl, theops )
	{
		if ( theoption == "edit" )
		{
			...
			for ( var c = 0; c < ops.length; ++c )
				$('#ck_op_'+ops[c]).prop( 'checked', true ) ;
		...
		else if ( theoption == "delete" )
		{
			if ( confirm( "Delete this External URL?" ) )
				location.href = "extras.php?action=delete&extid="+theextid ;
		}
	}
	...
	function gen_new_key()
	{
		...
		{
			location.href = "extras.php?action=submit&jump=apis&"+unique ;
		}
	}
...
<?php include_once( "./inc_header.php" ) ?>
		...
		<div style="display: none; margin-top: 25px;" id="extras_external">
			<div>
				External URLs allow operators to easily access a webpage within the operator console (iframe method).  URLs could be for "Client Search", "Trial Accounts", "Company Directory", "Order Search" or other pages.  The links will appear on the operator console footer menu.
			</div>
			...
			<?php if ( count( $operators ) ): ?>
				...
				<div style="padding: 5px; margin-top: 55px;">
					...
					<div style="margin-top: 10px;">
						<form method="POST" action="extras.php" id="theform">
						<input type="hidden" name="action" value="submit">
						<input type="hidden" name="jump" value="external">
						<input type="hidden" name="extid" id="extid" value="0">
						<table cellspacing=0 cellpadding=5 border=0>
						<tr>
							<td>Name<br><input type="text" class="input" name="name" id="name" size="50" maxlength="15" value=""></td>
						</tr>
						<tr>
							<td>Target URL<br><input type="text" class="input" name="url" id="url" size="120" maxlength="255" value=""></td>
						</tr>
						<tr>
							<td>
								...
								<div class="li_op_focus round"><input type="checkbox" id="ck_op_all" name="opids[]" value="all" onClick="check_all(this)"> <label for="ck_op_all" style="cursor: pointer;">Check All</label></div>
								<?php
									for ( $c = 0; $c < count( $operators ); ++$c )
									{
										...
										if ( $operator["name"] != "Archive" )
											print "<div class=\"li_op round\"><input type=\"checkbox\" id=\"ck_op_$operator[opID]\" name=\"opids[]\" value=\"$operator[opID]\" onClick=\"reset_check_all()\"> <label for=\"ck_op_$operator[opID]\" style=\"cursor: pointer;\">$operator[name]</label></div>" ;
									}
								?>
			...
			<?php else: ?>
			<div style="margin-top: 15px;"><span class="info_error"><img src="../pics/icons/warning.png" width="12" height="12" border="0" alt=""> Add an <a href="ops.php" style="color: #FFFFFF;">Operator</a> to continue.</span></div>
			<?php endif ;?>
		</div>
		...
		<div style="display: none; margin-top: 25px;" id="extras_apis">
			...
			<div class="info_info">
				...
				<tr>
					<td><div><span class="info_clear" style="padding: 15px;">API Key: <input type="text" class="input" id="text_api_key" size="10" value="<?php echo $CONF["API_KEY"] ?>" onMouseDown="setTimeout(function(){ $('#text_api_key').select(); }, 200);" readonly></span></div></td>
					<td style="padding-left: 15px;"><div style=""><button type="button" onClick="gen_new_key()" class="btn">Generate New API Key</button> &nbsp; The APIs require an API Key to process the action.</div></td>
				</tr>
				</table>
			</div>

			<div style="margin-top: 25px;">
				<div>The following API URL returns a 1 (department is online) or 0 (department is offline).</div>
				<div style="margin-top: 5px;">
					<table cellspacing=0 cellpadding=0 border=0>
					<tr>
						<td class="td_dept_td">
							<select name="deptid" id="deptid" style="width: 200px; font-size: 16px;" onChange="switch_dept_api( this.value )">
				...
				<div style="margin-top: 25px;">The following API URL will return the number of visitors on your website. (<a href="code.php">Standard HTML Code</a> must be on your webpages to track the visitor count)</div>
				<div style="margin-top: 5px;">
					<table cellspacing=0 cellpadding=0 border=0>
					<tr>
						<td class="td_dept_td">
							<select style="width: 200px; font-size: 16px;">
							<option value="0">All Departments</option>
							</select>
						</td>
				...
				<div style="margin-top: 25px;">The following API URL will return the IP's GeoIP location.  The output will be in the format of: <code>country abbreviation,country name,region,city,latitude,longitude</code></div>
				<div style="margin-top: 5px;">
					<table cellspacing=0 cellpadding=0 border=0>
					<?php if ( $geoip ): ?>
					<tr>
						<td class="td_dept_td">
							<select name="f" id="f" style="width: 200px; font-size: 16px;" onChange="switch_geoip_api( this.value )">
					...
					<?php else: ?>
					<tr><td class="td_dept_td" style="text-shadow: none;"><span class="info_error"><img src="../pics/icons/warning.png" width="12" height="12" border="0" alt=""> <a href="extras_geo.php" style="color: #FFFFFF;">GeoIP Addon must be enabled.</a></span></td></tr>
					<?php endif ; ?>
					</table>
				</div>
			</div>
