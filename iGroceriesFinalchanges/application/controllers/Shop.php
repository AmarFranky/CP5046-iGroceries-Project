...
class Shop extends CI_Controller {
	...
	public function check()
	{
		...
		if(!empty($_POST['stripeToken']))
		{
			...
			//set api key
			$stripe = array(
			  "secret_key"      => "sk_test_8g7RJVY2Kqqp0MDjCvsc3O6F00TJRv46fi",
			  "publishable_key" => "pk_test_ZhDZS2H0j6oWf5blwtud6yxm00PyAGspb9"
			);
	...
	public function cust_profile()
	{
		...
		$data['profile']=$this->ShopModel->get_profile($id);
		$this->load->view('user/header');
		$this->load->view('user/cust_profile',$data);
		$this->load->view('user/footer');
	}
	public function search_res()
	{
		$pname=$this->input->post('pname');
		$data['items']=$this->ShopModel->search_products($pname);
		$this->load->view('user/header');
		$this->load->view('user/browse_items',$data);
		$this->load->view('user/footer');
	}
