	...
	if ( $action == "create" )
	{
		...
		if ( $token_ces == $token_ces_ )
		{
			...
			if ( isset( $deptinfo["deptID"] ) &&  ( $vname != "null" ) )
			{
				...
				LIST( $requestid, $request_country ) = Chat_put_Request( $dbh, $deptid, $opid, 0, 0, 0, $vses, $os, $browser, $ces, $resolution, $vname, $vemail, $ip, $vis_token_embed, $vis_token, $onpage, $title, $question, $marketid, $refer, $rstring, $custom, $auto_pop, $sim_ops ) ;
				if ( isset( $requestid ) )
				{
					...
					if ( !is_file( "$CONF[CHAT_IO_DIR]/$ces.txt" ) ) { UtilChat_AppendToChatfile( "$ces.txt", base64_encode( $text ) ) ; }
