<?php
	...
	else if ( $action === "op2op" )
	{
		...
		if ( isset( $opinfo["opID"] ) )
		{
			...
			LIST( $requestid, $request_country ) = Chat_put_Request( $dbh, $deptid, $opid, 0, 0, $opid_cookie, 0, $os, $browser, $ces, $resolution, $opinfo["name"], $opinfo["email"], $ip, "", "op2op", "op2op", "", "Operator to Operator Chat Request", 0, "", "", "" ) ;
			if ( isset( $requestid ) )
			{
