<?php
	...
	FUNCTION Ops_update_itr_IdleOps( &$dbh )
	{
		...
		/************************************************/
		// mobile app idle feature inactive at this time.
		/************************************************/
		$VARS_MOBILE_EXPIRED_OPS = 2160 ; // 3 months for now (hours) basically disabled
		...
		if ( $dbh[ 'ok' ] )
		{
			if ( database_mysql_nresults( $dbh ) )
			{
				...
				for( $c = 0; $c < count( $operators ); ++$c )
				{
					...
					$query = "INSERT IGNORE INTO p_opstatus_log VALUES( $now, $opid, 0, 0 )" ;
					database_mysql_query( $dbh, $query ) ;
					...
					if ( $operator["mapp"] )
					{
						if ( is_file( "$CONF[TYPE_IO_DIR]/$opid.mapp" ) )
						{
							...
							if ( isset( $VARS_MAPP_DEBUG ) ) { Util_Format_DEBUG( "[ mapp ] [x] $opid - update_itr.php [1]\r\n" ) ; }
						}
			...
			for ( $c = 0; $c < $total_dir_files; ++$c )
			{
				...
				if ( $opid )
				{
					...
					if ( $auto_offline_counter && ( $online_counter == $auto_offline_counter ) )
					{
						...
						{
							@unlink( "$CONF[TYPE_IO_DIR]/$opid.mapp" ) ;
							if ( isset( $VARS_MAPP_DEBUG ) ) { Util_Format_DEBUG( "[ mapp ] [x] $opid - update_itr.php [2]\r\n" ) ; }
						}
						if ( isset( $mapp_array[$opid] ) ) { $arn = $mapp_array[$opid]["a"] ; $platform = $mapp_array[$opid]["p"] ; }
