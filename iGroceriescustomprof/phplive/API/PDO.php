<?php
	...
	if ( !isset( $connection ) )
	{
		...
		try {
			...
			if ( !defined( 'API_Util_Error' ) )
				include_once( "$CONF[DOCUMENT_ROOT]/API/Util_Error.php" ) ;
			ErrorHandler( 600, "MySQL DB Connection Failed", $PHPLIVE_FULLURL, 0, Array() ) ; exit ;
		}
	}

	FUNCTION database_mysql_query( &$dbh, $query )
	{
		global $PHPLIVE_URI ; global $VARS_DB_ERROR_LOG_FILE ; global $VARS_MYSQL_THROTTLE_PAUSE ;
