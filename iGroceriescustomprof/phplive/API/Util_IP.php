<?php
	...
	FUNCTION Util_IP_GetIP( $token )
	{
		...
		{
			// IE has trouble reading some cookie naming conventions.  phplive_vid was renamed to phplivevid but
			// still check for it just in case for past visitors
			if ( !isset( $_COOKIE["phplivevid"] ) && !isset( $_COOKIE["phplive_vid"] ) )
			{
				...
				if ( $token )
				{
					...
					$_COOKIE["phplivevid"] = $cookie_vid ;
					Util_Format_SetCookie( "phplivevid", $cookie_vid, time()+(60*60*24*3650), "/", "", $PHPLIVE_SECURE ) ;
				}
				else { $cookie_vid = $cookie_fallback ; }
			}
			else { $cookie_vid = ( isset( $_COOKIE["phplive_vid"] ) ) ? $_COOKIE["phplive_vid"]  : $_COOKIE["phplivevid"] ; }
		} else { $cookie_vid = $cookie_fallback ; }
		return Array( $ip, $cookie_vid ) ;
	}
