<?php
	...
	$at = Util_Format_Sanatize( Util_Format_GetVar( "at" ), "ln" ) ; // Auto Task
	...
	if ( !isset( $VALS['POPOUT'] ) ) { Util_Vals_WriteToFile( "POPOUT", "on" ) ; }
	...
	else if ( $action === "submit" )
	{
		...
		else
		{
			...
			if ( isset( $opinfo["opID"] ) && ( $password == md5( md5( "phplive".substr( md5( $CONF['SALT'].$opinfo["password"] ), 0, 6 ) ).$token_pass ) ) )
				...
				else
				{
					...
					if ( $mapp )
					{
						...
						if ( is_file( "$CONF[TYPE_IO_DIR]/$mapp_opid.mapp" ) )
						{
							...
							if ( isset( $VARS_MAPP_DEBUG ) ) { Util_Format_DEBUG( "[ mapp ] [x] $mapp_opid - index.php [1]\r\n" ) ; }
						}
					...
					if ( is_file( "$CONF[CONF_ROOT]/autotask_config.php" ) ) { include_once( "./addons/autotask/patch.php" ) ; }
...
<link rel="prefetch" href="./pics/bg_btn_focus.gif">
...
<!--
	...
	*/
	...
	$(document).ready(function()
	{
		...
		<?php
			...
			else if ( ( $action === "submit" ) && ( $menu == "operator" ) && !$error )
			{
				...
				else
				{
					...
					print "input_disable() ; $('#btn_login').attr('disabled', true).html('Logging in... <img src=\"pics/loading_fb.gif\" width=\"16\" height=\"11\" border=\"0\" alt=\"\"> ') ; document.title = \"-- Chat Operator Login --\" ; " ;
					if ( ( $wp || $auto ) && !$pr_query )
		...
		<?php if ( preg_match( "/inactive/i", $error ) ): ?>$('#div_alert_inactive').show() ;
...
<div id="body" style="padding-bottom: 20px;">
	<div style="width: 100%; padding-top: 25px;">
		<div style="<?php echo ( $mapp ) ? "display: none;" : "" ?> width: 280px; margin: 0 auto;">
			<div style="font-size: 14px; color: #ABB7C2; text-shadow: 1px 1px #EBEFF2; text-align: center;" id="chat_text_version"><?php if ( isset( $CONF["KEY"] ) && ( $CONF["KEY"] == md5($KEY."_key_phplive") ) ): ?>Live Chat Solution<?php else: ?>PHP Live! Support<?php endif ; ?> v.<?php echo $VERSION ?></div>
		</div>

		<div style="width: 280px; height: 370px; margin: 0 auto; margin-top: 20px; padding: 10px;" id="div_login">
			...
			<tr>
				...
				<td> <input type="text" class="input" name="phplive_login" id="phplive_login" size="15" maxlength="25" value="<?php echo ( $login ) ? $login : "" ?>" onKeydown="input_text_listen(event);" autocomplete="off" style="outline: none;"></td>
			</tr>
			<tr id="div_tr_password">
				<td width="60">Password</td>
				<td> <input type="password" class="input" name="phplive_password_temp" id="phplive_password_temp" size="15" value="<?php echo ( isset( $password ) && $reload ) ? $password : "" ; ?>" onKeydown="input_text_listen(event);" autocomplete="off" style="outline: none;"></td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan=3>
					...
					<div id="div_btn_submit" style="padding-top: 15px;">
						<button type="button" id="btn_login" onClick="do_login()" class="btn round" style="width: 100%; height: 45px; padding: 6px; font-size: 14px; font-weight: bold; background: #8BCF92; border: 1px solid #7FBD85; color: #FFFFFF; cursor: pointer;">Click to Log In</button>
						<div style="margin-top: 25px; text-align: center;"><a href="JavaScript:void(0)" onClick="toggle_forgot(1)" id="href_forgot"></a></div>
					</div>
		...
		<div style="padding-top: 15px;">
			<div id="chat_text_powered" style="width: 280px; margin: 0 auto; font-size: 10px; text-shadow: 1px 1px #FFFFFF; text-align: center;">
