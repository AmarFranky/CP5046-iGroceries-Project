...
<!--
	...
	function populate_vinfo( theces )
	{
		if ( parent.chats[theces] != "undefined" )
		{
			...
			if ( !parent.chats[theces]["tag"] )
				$('#req_tag').empty().html( parent.$('#req_tag').html() ) ;
	...
	function populate_ops()
	{
		...
		for ( var c = 0; c < json_length; ++c )
		{
			...
			var btn_transfer_department = ( total_dept_ops ) ? "&nbsp;<button type=\"button\" onClick=\"parent.transfer_chat( "+departments[c]["deptid"]+",'"+departments[c]["name"]+"', 0, 0, 1);$(this).attr('disabled', 'true');\" style=\"font-size: 10px;\">transfer</button>" : "" ;
			...
			for ( var c2 = 0; c2 < total_dept_ops; ++c2 )
			{
				...
				if ( departments[c].operators[c2]["status"] )
				{
					...
					btn_transfer = "<button type=\"button\" onClick=\"parent.transfer_chat( "+departments[c]["deptid"]+",'"+departments[c]["name"]+"',"+departments[c].operators[c2]["opid"]+",'"+departments[c].operators[c2]["name"]+"', 1);$(this).attr('disabled', 'true');\" style=\"font-size: 10px;\">transfer</button>" ;
				}
